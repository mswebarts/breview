...
function msbr_breview_general_settings_page() {
	...
	if ( isset( $_POST['msbr_general_form_submitted'] ) ) {
		// check nonce.
		if ( ! isset( $_POST['msbr_general_form_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['msbr_general_form_nonce'] ) ), 'msbr_general_form_action' ) ) {
			return;
		}
		$submitted = sanitize_text_field( wp_unslash( $_POST['msbr_general_form_submitted'] ) );
		...
		if ( 'Y' === $submitted ) {
			...
			if ( isset( $_POST['msbr_review_list_header_design'] ) ) {
				$review_list_header = sanitize_text_field( wp_unslash( $_POST['msbr_review_list_header_design'] ) );
			} else {
			...
			if ( isset( $_POST['msbr_review_list_design'] ) ) {
				$review_list_design = sanitize_text_field( wp_unslash( $_POST['msbr_review_list_design'] ) );
			} else {
...
function msbr_breview_email_settings_page() {
	...
	if ( isset( $_POST['msbr_email_form_submitted'] ) ) {
		// check nonce.
		if ( ! isset( $_POST['msbr_email_form_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['msbr_email_form_nonce'] ) ), 'msbr_email_form_action' ) ) {
			return;
		}
		$submitted = sanitize_text_field( wp_unslash( $_POST['msbr_email_form_submitted'] ) );
